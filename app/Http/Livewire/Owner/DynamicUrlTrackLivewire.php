...
class DynamicUrlTrackLivewire extends Component
{
    ...
    public function fetchLineChartData()
    {
        ...
        ->groupBy('month_year', 'name')
        ->get();
    
    $monthsInYear = [];
    for ($month = 1; $month <= 12; $month++) {
        $monthsInYear[sprintf('%04d-%02d', now()->year, $month)] = 0;
    }
    
    $this->lineChartData = $lineChartData->groupBy('name')->map(function ($data, $name) use ($monthsInYear) {
        $monthlyCounts = $data->pluck('count')->toArray();
    
        // Initialize an array to store sorted results
        $sortedChartData = array_replace($monthsInYear, array_combine($data->pluck('month_year')->toArray(), $monthlyCounts));
    ...
    $this->emit('lineChartDataUpdated', $this->lineChartData);
    ...
    public function render()
    {
        ...
        $cols_th = ['#', 'Name', 'Redirect URL', 'IPv4', 'Device Name', 'Visit Date', 'Visit Time', 'Action'];
        $cols_td = ['id', 'name', 'redirect_url', 'ip_Address', 'device_identifier', 'visit_date', 'visit_time','Action'];
